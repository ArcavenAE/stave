...
2026-08-09: that credential now exists (profile `measurement`, twelve
scopes, no `read:all`), and the names claim is on evidence for the first
time rather than by inference (finding-008). Assignment is now READABLE
rather than inferable: the server names an operation's required scopes in
its denials, so an under-privileged credential reads them off at one call
each (finding-009, `question-denial-driven-scope-discovery`). One
assignment is server-validated. `SCOPE_METADATA_PROVISIONAL` stays `true`
until the rest are run.
...
|---------|------|----------|
| The measurement credential, and a denial worth more than a success | 2026-08-09 | **Three findings, one ticket closed, four filed, first least-privilege credential minted.** `aae-orc-oqg2.1` delivered and closed: `docs/design/measurement-account-request.md` carries the `createServiceAccount` request, executed once in the console, enrolled as profile `measurement` and verified at **twelve scopes, `read:all` absent, excess 0** against the 79-scope baseline. **finding-008**: all twelve declared scope names are literally present in the granted set, tested with no `read:all` implication, so the validated count goes from two to twelve; `aae-orc-h9uo`'s critique of finding-007's reasoning stands while its conclusion does not. Both `read:users` and `read:user_accounts` are real and granted, so the operator's correction was right and the open question is which governs `list_users`, not spelling. **finding-009**: `list_permission_scopes` was curated (`aae-orc-kuqt`) with a guessed `required_scopes`, deliberately one the credential lacked, and the server refused **naming what it wanted**: `[read:all read:permission_scopes]`. The guess was right, `read:all` is confirmed as an implication scope by the vendor, and per-operation assignment is READABLE rather than inferable, retiring finding-008's twelve-mint route. Filed as frontier `question-denial-driven-scope-discovery`. **finding-010**: both kinds `aae-orc-i8cj` rests on now return complete field sets under both credentials (`audit_log` 8/8, `cloud_account` 11/11), so a P1's premise does not reproduce; both cheap explanations checked and unavailable; ticket re-scoped from diagnose to watch, not closed. Silent scope stripping separately weakened: across three kinds the narrow credential sees exactly what the broad one sees. **`docs/design/role-credential-templates.md`**: six least-privilege templates derived from the runbook catalogue (spoke-team remediation 7, vulnerability analyst 6, compliance evidence 4, identity auditor 3, inventory reconciler 3, reporting consumer 1) against 79; identity auditor and spoke-team owner are nearly disjoint and both get `read:all` today; `list_issues` declaring `read:users` + `read:service_accounts` inflates three of six, and if that requirement is real then least privilege is capped by what our documents select. Also shipped: `scripts/scope-membership.sh`, the per-kind `--catalog` allowlist in `scrub.sh` with four selftests, `build.rs` re-stamping on same-branch commits (a locally built binary had been reporting a commit it did not contain), multi-profile enrolment fixes. Six coach-gated reads through `runlog.sh`, reconciled clean, zero bypasses. bd: `oqg2.1` closed; `98g6`, `7f5a`, `t5cd` filed; `i8cj`, `h9uo`, `aeq2`, `oqg2` updated. Ideas: `role-template-catalogues-for-provisioning`. Deferred by operator: project narrowing (`t5cd`), and the catalogue read, which the coach HALTed on the session read-count threshold. |
