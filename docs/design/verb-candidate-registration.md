...
Amendments to this registration are held in
`verb-scoring-amendments.md`, which is **gate-scoped**: it is read at
`.15`, `.7`, and `.8`, and NOT by the paper-pipeline author at `.14`.

They are separated because they necessarily discuss the sealed control
arm, and `.14`'s author must read sections 1 through 3 above for the verb
list and the predicate. Putting the two in one file would hand the
treatment arm a description of the control. Sections 1 through 3 are
complete and self-sufficient for writing pipelines; nothing in the
amendments changes N, M, the glue kinds, the decoy set, or the blind
scoring procedure.
